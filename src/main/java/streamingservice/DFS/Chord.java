...
public class Chord extends java.rmi.server.UnicastRemoteObject implements ChordMessageInterface
{
    ...
    public void put(long guidObject, String text) throws IOException, RemoteException {
        try {
            String fileName = "./"+guid+"/repository/" + guidObject;
            FileOutputStream output = new FileOutputStream(fileName);
            output.write(Base64.getDecoder().decode(text));
            output.close();
        } catch (IOException e) {
            System.out.println(e);
        }
    ...
    public RemoteInputFileStream get(long guidObject) throws RemoteException {
        ...
        try {
             file = new RemoteInputFileStream("./"+guid+"/repository/" + guidObject);
        } catch (IOException e)
        {
    ...
    @Override
    public byte[] get(long guidObject, int offset, int len) throws IOException, RemoteException {
        return new byte[0];
    }
...
//           e.printStackTrace();
    ...
    public void leave() {
        ...
        try {
            ChordMessageInterface succ = successor;

            if (succ != null && succ.getId() != getId()) {
                successor = null;
                predecessor.joinRing(succ);
                predecessor = null;
                ...
                try {
                    ...
                    for (File file : files) {
                        try {
                            long guid = Long.parseLong(file.getName());
                            succ.put(guid, new RemoteInputFileStream(file.getPath()));
